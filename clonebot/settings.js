...
msg.chat = ptz.decodeJid(msg.key.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || '')
msg.now = msg.messageTimestamp
msg.isGroup = msg.chat.endsWith('@g.us')
msg.sender = ptz.decodeJid(msg.key.fromMe && ptz.user.id || msg.participant || msg.key.participant || msg.chat || '')
}
if (msg.message) {
...
msg.msg.key.fromMe = ptz.decodeJid(msg.msg.key.participant) === ptz.decodeJid(ptz.user.id)
if (!msg.msg.key.fromMe && msg.msg.key.remoteJid === ptz.decodeJid(ptz.user.id)) msg.msg.key.remoteJid = msg.sender
}
msg.text = msg.msg || ''
...
msg.quoted.chat = ptz.decodeJid(msg.msg.contextInfo.remoteJid || msg.chat || msg.sender)
msg.quoted.isBaileys = msg.quoted.id && msg.quoted.id.length === 16 || false
msg.quoted.sender = ptz.decodeJid(msg.msg.contextInfo.participant)
...
if (msg.quoted.url || msg.quoted.directPath) msg.quoted.download = () => ptz.downloadMediaMessage(msg.quoted)
msg.quoted.copy = () => smsg(ptz, M.fromObject(M.toObject(vM)))
}
}
if (msg.msg && msg.msg.url) msg.download = (saveToFile = false) => ptz.downloadM(msg.msg, msg.mtype.replace(/message/i, ''), saveToFile)
return msg
}
